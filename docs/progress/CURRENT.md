...
- **A reload that lands exactly on a phase boundary can resubscribe after the broadcast and sit on stale staging until the next phase event.** Found in M3 P2a while stabilising the sudden-death e2e test (see [`M3-P2a-the-tiebreak.md`](M3-P2a-the-tiebreak.md)), reproduced twice: a reload straddling READ→ANSWER left `steps.optionsMode` at `dim` for a whole 20s ANSWER phase, so the grid never went live. **Not tiebreak-specific — any phase boundary can do it — and not fixed.** The shape of a real fix is `lib/useRoomChannel.ts` re-fetching `get_room_state` once after the channel reports `SUBSCRIBED`, since the phase event it missed is exactly what that call would replace.

- **The player route swallows a `get_room_state` error and hangs on "Connecting…" forever for an unknown or expired room code.** `app/room/[code]/page.tsx` destructures `{ data }` from the RPC call and discards `error`, so a bad code never surfaces — it just spins, unlike the stage route's `roomMissing` handling (`lib/store.ts`, `app/stage/[code]/page.tsx`). Flagged as out-of-scope carried-forward work in [`P6a-stage-view.md`](P6a-stage-view.md) ("adopting it on the player route is a separate improvement, deliberately out of this phase's scope") and never picked up since. The fix is the same shape the stage route already has: read `error` off the call and set `roomMissing`.
